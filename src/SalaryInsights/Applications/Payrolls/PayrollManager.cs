// -----------------------------------------------------------------------
// <copyright file= "PayrollManager.cs">
...
namespace SalaryInsights.Applications.Payrolls;

public class PayrollManager : BaseManager, IPayrollManager
{
    #region Initializes
    ...
    public async Task<OperationResponse<Guid, PayrollDetailsDto>> CreateAsync(PayrollCreationDto creationDto)
    {
        try
        {
            ...
            return OperationResponse<Guid, PayrollDetailsDto>.Success(payroll.Id,
        ...
        catch (Exception ex) when (ex is not SIException)
        {
            ...
            return OperationResponse<Guid, PayrollDetailsDto>.Failure("Failed to create the payroll.");
        }
    }

    public async Task<OperationResponse<Guid, PayrollDetailsDto>> UpdateAsync(PayrollEditDto editDto)
    {
        try
        {
        ...
        catch (Exception ex) when (ex is not SIException)
        {
            ...
            return OperationResponse<Guid, PayrollDetailsDto>.Failure("Failed to update the payroll.");
        }
    }

    public async Task<OperationResponse<Guid, PayrollDetailsDto>> DeleteAsync(Guid id)
    {
        try
        {
            ...
            return OperationResponse<Guid, PayrollDetailsDto>.Success(id,
                null);
        }
        catch (Exception ex) when (ex is not SIException)
        {
            _logger.LogError(ex, "An error occurred while deleting the payroll:{Id}", id);
            return OperationResponse<Guid, PayrollDetailsDto>.Failure("Failed to delete the payroll.");
        }
    }
